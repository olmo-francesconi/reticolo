"""Compare the three HMC integrators (Leapfrog, Omelyan2, Omelyan4) across two
regimes:
    - easy     (κ ≪ κ_c, deep in the symmetric phase, fast dynamics)
    - critical (κ ≈ κ_c, slow dynamics, where the integrator choice matters)
...
ALGO_ORDER = ["hmc_leapfrog", "hmc_omelyan2", "hmc_omelyan4"]
ALGO_SHORT = {
...
# -----------------------------------------------------------------------------
# Plot 4: summary bar per scenario
# -----------------------------------------------------------------------------
    ...
    for row_i, (name, _, rows) in enumerate(scenarios):
        ...
        for col, obs in enumerate(["S", "msq"]):
            ...
            for algo in ALGO_ORDER:
                ...
                vals.append(ess)
                errs.append(ess_err)
                labels.append(f"{ALGO_SHORT[algo]}\nτ={r['tau']:.1f}, n_md={int(r['n_md'])}")
