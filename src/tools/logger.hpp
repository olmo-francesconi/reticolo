...
inline std::string reticoloLogLineInit_time() {
  std::string Message = "reticolo..." +
                        std::format("{:.>10.3f}", GlobalTimer.elapsed_s()) +
                        " s | ";
  return Message;
};

/* Default reticolo log line init with dots */
inline std::string reticoloLogLineInit_dots() {
  std::string Message = "reticolo............... | ";
  return Message;
};

/* Default reticolo log line empty init */
inline std::string reticoloLogLineInit_void() {
  std::string Message = "                        | ";
  return Message;
};

/* Default reticolo log error line init */
inline std::string reticoloLogLineInit_erro() {
  std::string Message = "reticolo..........ERROR | ";
  return Message;
};

/* Default reticolo log warning line init */
inline std::string reticoloLogLineInit_warn() {
  std::string Message = "reticolo..........ERROR | ";
  return Message;
};
...
public:
  ...
  Logger(const std::filesystem::path &OutPath, const std::string &FileName,
  ...
  inline void init(const std::filesystem::path &OutPath,
                   const std::string &FileName,
                   const std::string &LogName = "") {
    if (state != 0) {
      throw std::runtime_error(
          std::string("reticolo...LOGGER ERROR | Logger (") + logger_name +
  ...
  inline void log_string(const std::string &who, const std::string &what) {
    if (state == 1) {
      file << reticoloLogLineInit_time() << who << " - " << what << '\n';
    } else {
      throw std::runtime_error("reticolo: LOGGER ERROR : Trying to write to an "
                               "uninitialized logger");
    }
  }

  inline void log_timing(const std::string &who, const std::string &what,
                         double time) {
    if (state == 1) {
      file << reticoloLogLineInit_time() << who << " - " << what << " in "
           << std::format("{:>8.2f}", time) << " ms" << '\n';
    } else {
      throw std::runtime_error("reticolo: LOGGER ERROR : Trying to write to an "
                               "uninitialized logger");
    }
  ...
  inline void log_memory(const std::string &who, const std::string &what,
                         size_t memory) {
    if (state == 1) {
      file << reticoloLogLineInit_time() << who << " - " << what << " "
           << pretty_bytes(memory) << '\n';
  ...
  inline void log_threadig(const std::string &who, size_t nThreads) {
    if (state == 1) {
      file << reticoloLogLineInit_time() << who << " - Running on " << nThreads
           << " threads" << '\n';
    } else {
      throw std::runtime_error("reticolo: LOGGER ERROR : Trying to write to an "
                               "uninitialized logger");
