...
// identity cold-start and gauge WindowedAction machinery are validated on SU(2),
// and SU(3) reuses the same generic gauge_sun kernels. Cold = SU(3) identity via
// set_cold; no hot-start (smooth confined at the default β).
//
// Output schema (HDF5): identical to phi4_llr_cuda / su2_llr_cuda.
...
int main(int argc, char** argv) {
    ...
    cli::Parser p{"su3_llr_cuda", "LLR (Gaussian-penalty) + replica exchange for SU(3) Wilson (CUDA)"};
    auto const cf     = app::common_flags(p, {.L = 4, .out = "su3_llr_cuda.h5"});
    auto const& ndim  = p.opt<int>("ndim", 4, "spatial dimensions");
    auto const& beta  = p.opt<double>("beta", 6.0, "Wilson coupling");
    auto const& e_min = p.opt<double>("E_min", 1000.0, "lower window centre");
    auto const& e_max = p.opt<double>("E_max", 4000.0, "upper window centre");
    auto const& delta = p.opt<double>(
        "delta", 500.0, "Gaussian penalty width δ in (S−E_n)²/2δ² (also the a-update scale)");
    ...
    // ---- Replicas (cold = SU(3) identity via set_cold; warmed into window) ----
