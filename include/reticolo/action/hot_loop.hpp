...
inline void visit_nn_2d_(Lattice<T> const& l, Body&& body) noexcept {
    ...
    for (std::size_t y = 0; y < Ly; ++y) {
        ...
        // x = 0
        {
            ...
            body(i, data[i], data[i + 1] + data[row + (Lx - 1)]
                            + data[row_yp] + data[row_ym]);
        }
        // x in (0, Lx-1) — bulk
        for (std::size_t x = 1; x + 1 < Lx; ++x) {
            std::size_t const i = row + x;
            body(i, data[i], data[i + 1] + data[i - 1]
                            + data[row_yp + x] + data[row_ym + x]);
        }
        // x = Lx-1
        {
            std::size_t const i = row + (Lx - 1);
            body(i, data[i], data[row] + data[i - 1]
                            + data[row_yp + (Lx - 1)] + data[row_ym + (Lx - 1)]);
        }
    }
}
        ...
        for (std::size_t z = 0; z < L2; ++z) {
            ...
            for (std::size_t y = 0; y < L1; ++y) {
                ...
                // x = 0
                {
                    std::size_t const i = row;
                    body(i, data[i],
                         data[i + 1] + data[row + (L0 - 1)] +
                         data[row_yp] + data[row_ym] +
                         data[row_zp] + data[row_zm] +
                         data[row_wp] + data[row_wm]);
                }
                // x in (0, L0-1) — bulk
                ...
                // x = L0-1
                {
                    ...
                    body(i, data[i],
                         data[row] + data[i - 1] +
                         data[row_yp + (L0 - 1)] + data[row_ym + (L0 - 1)] +
                         data[row_zp + (L0 - 1)] + data[row_zm + (L0 - 1)] +
                         data[row_wp + (L0 - 1)] + data[row_wm + (L0 - 1)]);
                }
            }
        }
    }
...
#else
    switch (l.ndims()) {
        case 1: visit_nn_1d_(l, std::forward<Body>(body)); return;
        case 2: visit_nn_2d_(l, std::forward<Body>(body)); return;
        case 3: visit_nn_3d_(l, std::forward<Body>(body)); return;
        case 4: visit_nn_4d_(l, std::forward<Body>(body)); return;
        default: visit_nn_fallback_(l, std::forward<Body>(body)); return;
    }
#endif
}
...
[[nodiscard]] inline T reduce_fwd_3d_(Lattice<T> const& l, Body&& body) noexcept {
    ...
    for (std::size_t z = 0; z < Lz; ++z) {
        ...
        for (std::size_t y = 0; y < Ly; ++y) {
            ...
            std::size_t const i = row + (Lx - 1);
            total += body(data[i],
                          data[row] + data[row_yp + (Lx - 1)] + data[row_zp + (Lx - 1)]);
        }
    }
    return total;
}
        ...
        for (std::size_t z = 0; z < L2; ++z) {
            ...
            for (std::size_t y = 0; y < L1; ++y) {
                ...
                for (std::size_t x = 0; x + 1 < L0; ++x) {
                    std::size_t const i = row + x;
                    total += body(data[i],
                                  data[i + 1] + data[row_yp + x] +
                                  data[row_zp + x] + data[row_wp + x]);
                }
                std::size_t const i = row + (L0 - 1);
                total += body(data[i],
                              data[row] + data[row_yp + (L0 - 1)] +
                              data[row_zp + (L0 - 1)] + data[row_wp + (L0 - 1)]);
            }
        }
    }
    return total;
...
#else
    switch (l.ndims()) {
        case 1: return reduce_fwd_1d_(l, std::forward<Body>(body));
        case 2: return reduce_fwd_2d_(l, std::forward<Body>(body));
        case 3: return reduce_fwd_3d_(l, std::forward<Body>(body));
        case 4: return reduce_fwd_4d_(l, std::forward<Body>(body));
        default: return reduce_fwd_fallback_(l, std::forward<Body>(body));
    }
#endif
}
