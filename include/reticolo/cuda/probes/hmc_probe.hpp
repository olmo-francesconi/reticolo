...
// HMC gates (defined in src/cuda/hmc_probe.cu).
...
// A graph-captured MD trajectory (and its replay) reproduces the eager MD
// field bit-for-bit from the same (q0, p0).
[[nodiscard]] bool graph_replay_matches_eager();

// K host-free trajectories (graph replay + device-side Metropolis accept,
// no per-step sync) are deterministic — two runs from the same seed agree
// bit-for-bit — with a sane acceptance and a finite field.
