...
`reticolo` is a C++20 header-mostly library for Hybrid Monte Carlo simulations of lattice quantum field theories (scalar fields, U(1)/SU(2)/SU(3) gauge), plus a small set of hand-written reference apps under `apps/`. HMC is the only update algorithm; LLR (Logarithmic Linear Relaxation) is an orchestration layer on top of it. Single-replica and LLR replica ensembles. OpenMP threads the CPU at three composable layers (see §CPU parallelism): the lattice-traverse stage inside a single trajectory, the per-trajectory HMC region, and the LLR replica ensemble. Without `-fopenmp` every layer degrades to serial.
...
```
include/reticolo/      header-only core, actions, algorithms, observers, LLR, CLI shim, app-setup helpers, optional CUDA backend
...
Inside `include/reticolo/`: `core/` (indexing, lattices, RNG, logger, `parallel.hpp` = the `exec::` threading primitives), `action/` (the four action families + sweep engine + concepts), `algorithm/` (`alg::Hmc` + integrators), `llr/` (replica orchestration + drivers), `obs/`, `app/` (shared pre-loop scaffolding: `app::common_flags` + `app::open_writer`), `cli/`, `math/` (gauge group math), `io/`, `cuda/` (optional device backend).
...
- **No virtual dispatch in the hot loop.** Actions are plain structs satisfying the field-agnostic HMC concepts in `<reticolo/action/concepts.hpp>` — `HmcAction` (`s_full` + `compute_force`), with opt-in refinements `HasFusedKick` and `HasImagPart`. One concept set covers site (`Lattice<F>`) and gauge (`MatrixLinkLattice<G,F>`) actions alike. The sole updater `alg::Hmc<A,R,Integ>` is a class template constrained by `HmcAction`; `llr::Replica` is the orchestration layer on top. No base class, no `register_action`, no string dispatch.
- **HMC integrator is a type parameter**, not a runtime switch. `alg::integ::Leapfrog`, `Omelyan2`, `Omelyan4` are structs with a static `run(...)`. A new integrator = new struct + instantiate `Hmc<A, R, NewInteg>`.
- **CPU thread counts are per-`Hmc` state, not a global.** `HmcSpec.n_threads` / `slabs_per_thread` (0 = ambient) are bound each trajectory via `exec::team_scope` / `exec::slab_scope` so an `Hmc` can be pinned to a fixed team regardless of the OpenMP global — the basis for mixing replica-level and site-level parallelism. Results stay bit-identical for fixed `n_threads` + `slabs_per_thread` (deterministic fixed-partition reductions).
...
`reticolo::log` is a thread-safe (mutex around `std::cout`/`std::cerr`), OpenMP-aware logger. Severity = sigil (`·` debug, `┃` info, `⚠` warn, `✖` error), each line carries elapsed `HHH:MM:SS.mmm` + a 4-char tag. `log::start(workspace, out_name[, replicas])` is the single init: creates the workspace folder, opens `<workspace>/<stem>.log` (stem = out_name minus extension) and mirrors every entry into it, then prints the banner. `replicas=true` (LLR apps) adds the run-id column and per-replica `<stem>.<rNNN>.log` files. RAII `log::scope("rNNN")` binds a thread-local replica/run tag — `llr::Replica` binds its own id inside its public methods; apps/drivers bind a scope only when their own code logs inside a parallel region (the warm-up loops). Transitively-called code picks up the tag automatically.

Lattice / RNG / Writer / action / algorithm / `llr::Replica` constructors auto-announce themselves — most apps call `log::info` rarely. Tests and benchmarks call `log::off()` to short-circuit formatting.
...
- **Per-trajectory (`alg::Hmc`).** Each trajectory binds `team_scope{n_threads}` + `slab_scope{slabs_per_thread}` so the drift/kick/force passes thread under a fixed team (see the invariant above). Kernels are pure workers over a flat work-item index — they never see OpenMP.
...
`llr::Replica` wraps a base action in `llr::WindowedAction` (adds `a·S + (S − E_n)²/2δ²`). Newton-Raphson warm-up + Robbins-Monro adapt the per-replica `a`. Periodic even/odd replica exchange. Replicas run in parallel via `#pragma omp parallel for` over the replica vector (`llr/driver.hpp`, `llr/smoothed_driver.hpp`); each replica's HMC stays single-threaded inside that team.
...
- **Add a new action**: three steps — pick a family base, write the formula, wire it up. (1) Pick the family base by interaction shape: `SiteAction<D,T>` (self + NN sum), `BondAction<D,T>` (endpoint-difference, e.g. XY), `ComplexAction<D,T>` (S_R + i·S_I sign problem), or `GaugeAction<D>` (link field; `Wilson<G>` derives from it — for a new gauge *theory* you usually just write a group model `G` and use `Wilson<G>`). (2) Put **all the physics** in a `RETICOLO_HD` formula file (`action/<family>/formula/<name>_formula.hpp`) — the single source of truth shared with the CUDA functors. (3) Write a small aggregate leaf (`action/<family>/<name>.hpp`) `struct <Name> : SiteAction<<Name><T>, T>` with the couplings, `describe`, and coupling-hoisting kernels (`force_kernel`/`action_kernel`, or the bond/complex/gauge equivalents) that bind the formula. The base supplies `s_full`/`compute_force`/`compute_force_and_kick`/caches; gauge is parameterised by `G` instead of derived. Deriving keeps the leaf a designated-init aggregate, so `{.kappa=…}` still works. Add the header to the matching family aggregator (`action/<family>.hpp`). The `action/` tree is four families (site/bond/complex/gauge), each with leaves + the family base (`<family>_action.hpp`) at top + `formula/` (physics); the shared dimension-generic traversal engine lives once in `action/sweep/` (namespace `action::sweep`); concepts live in `action/concepts.hpp`, gauge groups (core group math) in `math/group/` (namespace `math::group`), their Wilson kernels in `action/gauge/formula/`. See `docs/writing_an_app.md` §Adding an action. Concept failures at the `alg::Hmc` site point at the missing member — read the error.
