...
Each `tests/{unit,physics,io}/*.cpp` is registered via `reticolo_add_test(<name> <category> <source>)` in `tests/CMakeLists.txt`; tests are linked against a shared `tests/test_main.cpp` that calls `log::off()` so test output stays clean. The category is mandatory (`core rng physics io app cuda`, unknown = configure error) and becomes a CTest label, so `ctest -L physics` works and `tools/check.sh` can report pass/fail per category.
...
Inside `include/reticolo/`: `core/` is organised into subfolders — `core/field/` (lattices, `indexing`, `site`, `cplx`, `field_traits`), `core/exec/` (`parallel.hpp` = the `exec::` per-site threading/partition primitives, `field_ops.hpp` = the elementwise field atoms `drift_field`/`kick_add`/`scale_field` — re-exported into `updater::integ` and overloaded for `DeviceField` in `reticolo::cuda`, **plus** `nn_stencil.hpp`/`nn_site.hpp`/`nn_split.hpp` = the dimension-generic neighbour stencil, the `exec::nn_*` family), `core/rng/`, `core/log/`, `core/sys/` (build/host metadata + the `RETICOLO_HD` macro). `action/` (the action families — `nn/` scalar nearest-neighbour (`NNAction`, covers site + bond), `complex/` sign-problem (real base + `ImagPart` mixin), `gauge/`; plus `windowed_action.hpp` (a decorator action — base + Gaussian window on a `Constraint` observable, used by LLR but not LLR-specific), and concepts — the shared NN sweep engine now lives in `core/exec/`, not here), `updater/` (the update-algorithm subsystem: the `updater::Updater` concept at top + `updater/hmc/` = `updater::Hmc` + integrators; folder mirrors `action/` — one flat `updater::` namespace), `orch/` (the orchestration subsystem: a physics-free spine — `Worker`/`Checkpointable` concepts, `ThreadPlan`/`plan_threads`, `parallel_workers`, ensemble checkpoint — plus concrete orchestrators in `orch/span/` (parameter span) and `orch/llr/` (replica-exchange LLR, namespace `orch::llr`)), `obs/`, `app/` (shared pre-loop scaffolding, two tiers: `app::common_flags` + `app::open_writer` for every app, then ONE helper per algorithm family — `app::hmc_run_flags` + `app::resume_or_start` for HMC, `app::llr_run_flags` for LLR; a family helper is all-or-nothing), `cli/`, `math/` (gauge group math; `math/group/formula/` holds the `RETICOLO_HD` per-link SU(N) algebra shared with the device path), `io/`, `cuda/` (optional device backend).
...
- **Lattice-traverse (site-level).** `parallel_map` / `parallel_reduce` (and their `_ranges` variants) are the only holders of `#pragma omp for`. The neighbour stencil engine (`core/exec/nn_stencil.hpp`, the `exec::nn_*` family) drives `compute_force` / `s_full` through them. `in_traverse_region(want, body)` decides whether to open a region: it opens ONE `omp parallel` when `want` and not already nested; otherwise it worksplits (nested in our own region) or runs serial (nested in a foreign region, or the pass is too small). Chunking targets ~64 KiB working sets per work item.
...
- **Add a new test**: pick `tests/unit/`, `tests/physics/`, or `tests/io/`. Register it in `tests/CMakeLists.txt` with `reticolo_add_test(<name> <category> <relpath>)` — category is one of `core rng physics io app cuda` and is required. Shared harnesses exist and should be reused rather than re-rolled: `tests/physics/force_fd_helpers.hpp` (force-vs-FD for scalar + complex fields), `tests/apps/smoke_helpers.hpp` (HMC / LLR end-to-end HDF5 schema), `tests/apps/llr_resume_helpers.hpp` (bit-exact resume). Canonical patterns are in `docs/writing_an_app.md`.
