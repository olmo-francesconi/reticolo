...
```
include/reticolo/      header-only core, actions, algorithms, orchestration (spine + span + LLR), observers, CLI shim, app-setup helpers, optional CUDA backend
...
- **The updater has a contract.** `alg::Updater` (`algorithm/concepts.hpp`) is the sampler surface apps/orchestration depend on (`step` → `{dH, accepted}`, `last_s_full`, `rng`) — the updater-level analogue of `HmcAction`. `alg::Hmc` is the sole implementation; both orchestration workers `static_assert` their sampler against it. A new updater models the concept, reusing the `alg::integ::*` type-params — no base class.
...
Lattice / RNG / Writer / action / algorithm / `orch::llr::Replica` constructors auto-announce themselves — most apps call `log::info` rarely. Tests and benchmarks call `log::off()` to short-circuit formatting.
...
**Parameter span** (`orch::span`): `orch::span::Chain<Action,…>` is a generic HMC worker; `orch::span::run` sweeps a parameter grid concurrently (`apps/param_span_hmc`).

### LLR (`orch::llr`)

`orch::llr::Replica` wraps a base action in `orch::llr::WindowedAction` (adds `a·S + (S − E_n)²/2δ²`) and is an `orch::Worker`. Newton-Raphson warm-up + Robbins-Monro adapt the per-replica `a`. Periodic even/odd replica exchange. `orch::llr::run` / `smoothed::run` (`orch/llr/driver.hpp`, `orch/llr/smoothed_driver.hpp`) are clients of the spine — they drive `parallel_workers` and add exchange as a serial coupling between waves; each replica's HMC stays single-threaded inside the worker team.
