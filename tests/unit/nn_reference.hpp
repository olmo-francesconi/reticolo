...
// the neighbour tables are gone, the reference gathers via the COMPUTED
// the field's next/prev — same values, same fold order as the vectorised nests
...
inline void visit_nn_ref(Lattice<T> const& l, Body const& body) {
    ...
    for (std::size_t i = 0; i < l.nsites(); ++i) {
        ...
        for (std::size_t mu = 0; mu < nd; ++mu) {
            nb += d[l.next(Site{i}, mu).value()];
            nb += d[l.prev(Site{i}, mu).value()];
        }
        body(i, d[i], nb);
    }
}
