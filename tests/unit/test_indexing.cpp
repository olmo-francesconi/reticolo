...
TEST_CASE("Open BC yields invalid neighbours at the boundary", "[indexing]") {
    auto idx = Indexing::acquire({4, 4}, BcMask{Bc::Open, Bc::Periodic});
    REQUIRE_FALSE(idx->all_periodic());

    // (0,0): prev in mu=0 is invalid (open boundary), prev in mu=1 wraps periodic.
    REQUIRE_FALSE(idx->prev(Site{0}, 0).is_valid());
    REQUIRE(idx->prev(Site{0}, 1) == Site{12});

    // (3,0): next in mu=0 is invalid (open boundary).
    REQUIRE_FALSE(idx->next(Site{3}, 0).is_valid());
}
...
TEST_CASE("Bulk/skin partition is exhaustive and matches BCs", "[indexing]") {
    SECTION("all-periodic: everything is bulk") {
        auto idx = Indexing::acquire({4, 4});
        REQUIRE(idx->bulk_sites().size() == idx->nsites());
        REQUIRE(idx->skin_sites().empty());
    }
    SECTION("one open direction: skin is the 2 boundary slabs") {
        auto idx = Indexing::acquire({4, 4}, BcMask{Bc::Open, Bc::Periodic});
        REQUIRE(idx->bulk_sites().size() + idx->skin_sites().size() == idx->nsites());
        // On L=4 with one open direction, the skin slabs have width 1 on each end
        // => 2 * 4 = 8 boundary sites.
        REQUIRE(idx->skin_sites().size() == 8);
    }
    SECTION("two open directions: corners are double-counted into one skin set") {
        auto idx = Indexing::acquire({4, 4}, BcMask{Bc::Open, Bc::Open});
        // Skin = all sites with any coord on the boundary
        //      = 4*4 - 2*2 (interior 2x2 block) = 12.
        REQUIRE(idx->skin_sites().size() == 12);
        REQUIRE(idx->bulk_sites().size() == 4);
    }
}
...
TEST_CASE("Even + odd sites partition the lattice", "[indexing]") {
    ...
    // For even side-lengths, exact halves.
...
TEST_CASE("Indexing rejects mismatched BcMask dims", "[indexing]") {
    REQUIRE_THROWS_AS(Indexing::acquire({4, 4}, BcMask{Bc::Periodic, Bc::Open, Bc::Open}),
