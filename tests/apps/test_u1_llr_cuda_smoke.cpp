...
#ifndef U1_LLR_CUDA_BINARY
    #error "U1_LLR_CUDA_BINARY compile definition is required"
#endif
...
// End-to-end: the CUDA compact-U(1) LLR app writes the LLR schema (per-slot a /
// dE / E_n series + exchange counts) — the gauge twin of the phi4 LLR smoke.
// Requires a GPU at run time. Exercises the generic gauge WindowedAction over a
// LinkLayout field plus the device hot-start. Mirrors test_phi4_llr_cuda_smoke.
TEST_CASE("u1_llr_cuda binary writes the expected HDF5 schema", "[app][e2e][cuda][u1_llr]") {
    auto const out = scratch_path("u1_llr_cuda_smoke");
    std::error_code ec;
