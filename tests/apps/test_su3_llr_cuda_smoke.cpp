...
#ifndef SU3_LLR_CUDA_BINARY
    #error "SU3_LLR_CUDA_BINARY compile definition is required"
#endif
...
// End-to-end: the CUDA SU(3) Wilson LLR app writes the LLR schema. Exercises the
// gauge WindowedAction over a MatrixLayout<SU3> field plus the SU(3) identity
// cold-start (set_cold). Requires a GPU at run time. Mirrors
// test_su2_llr_cuda_smoke.
TEST_CASE("su3_llr_cuda binary writes the expected HDF5 schema", "[app][e2e][cuda][su3_llr]") {
    auto const out = scratch_path("su3_llr_cuda_smoke");
    std::error_code ec;
    std::filesystem::remove(out, ec);

    // Tiny config: 2D, 4 replicas, 2 NR, 2 RM with very short sample counts.
    std::string const cmd = std::string{SU3_LLR_CUDA_BINARY} + " --size=4 --ndim=2 --beta=5.0" +
                            " --E_min=8 --E_max=32 --delta=8" + " --tau=1.0 --n_md=10" +
